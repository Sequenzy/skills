...
- accounts (B2B organizations: companies, workspaces, teams) with `accounts list|get|upsert|delete|members|add-member|remove-member|event|events`; `upsert` sets `--name`, `--domain`, and repeatable `--attr key=value` attributes that fan out to every member as `account.<name>` for segments and `{{account.<name>}}` merge tags; members carry a role (`owner`, `admin`, `member`); `accounts event` records an account event and delivers it to `--recipients owners|admins|all|none` as a normal contact event so sequences and sync rules run per person; `subscribers event` accepts an account reference via the API `account` field to attach a contact (MCP: `list_accounts`, `get_account_by_external_id`, `upsert_account`, `delete_account`, `list_account_members`, `add_account_member`, `remove_account_member`, `trigger_account_event`, `list_account_events`)
...
- ab-tests `list`, `get`, `stats`, `restart`, `update-variant`, `create`, `add-variant`, `delete-variant`, and `delete`; create/add-variant/delete-variant/delete work on campaign A/B tests in draft status, variant A is the protected control, and `restart` reruns a finished sequence A/B test. For a sequence A/B step, `sequences get --json` and MCP `get_sequence` expose each variant's full blocks when `ab_tests:read` is granted; edit them only through `ab-tests update-variant` / `update_ab_test_variant`
